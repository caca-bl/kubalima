...
function showSection(id) {
    ...
    // Pastikan timer berhenti jika beralih section
    if (timerInterval) clearInterval(timerInterval);
    if (id !== "quiz") {
        const timerDisplay = document.getElementById('timer');
        if (timerDisplay) timerDisplay.textContent = 'Timer Dinonaktifkan';
    }
}
...
function toNet() {
    ...
    if (currentShapeName === "Kubus") {
        ...
        // 4. Left (d x h) - Di kiri Front. Posisi X: -(w / 2) - (d / 2)
        const posX_left = -(w / 2 + d / 2);
        targets.push({ pos: new THREE.Vector3(posX_left, 0, 0), quat: flatQuat });
        
        // 5. Right (d x h) - Di kanan Front. Posisi X: (w / 2) + (d / 2)
...
function showNetImage2D(shapeName) {
    ...
    if (shapeName === "Prisma") {
        img.src = "assets/images/prisma-segitiga.png"; // Asumsi Anda punya gambar jaring-jaring prisma
    } else if (shapeName === "Limas") {
        img.src = "assets/images/limas.jpg"; // Asumsi Anda punya gambar jaring-jaring limas
    } else {
        img.src = "";
    }
...
function isConnected(state) {
    ...
    if (faces.length === 0) return { connected: true, faces, faceCoords: [] };
    ...
    return { connected: visited.size === faces.length, faces, faceCoords };
}
...
function handleTileClick(e) {
    ...
    if (netBoardState[row][col] !== null) {
        // Jangan izinkan ubin pusat (CENTER_ROW, CENTER_COL) dihapus jika ada ubin lain yang terhubung
        const { faces } = isConnected(netBoardState);
        if (faces.length > 1 && row === CENTER_ROW && col === CENTER_COL) {
             alert("Ubin pusat tidak dapat dihapus jika ada ubin lain yang terhubung padanya.");
             return;
        }
...
/**
 * Memeriksa apakah susunan 6 muka persegi merupakan salah satu jaring-jaring kubus yang valid.
 * Jaring-jaring kubus (heksomino) memiliki 11 pola. Pola umum adalah 1-4-1, 2-3-1, 3-3, 2-2-2.
 * Kita akan memfokuskan pada cek pola 4-berurutan dan 3-berurutan.
 */
function isValidCubeNet(faceCoords) {
    if (faceCoords.length !== 6) return false;

    // 1. Cek Keterhubungan (Sudah dicek di isConnected, tapi harus 6 muka)
    // Asumsi: faceCoords adalah 6 muka yang terhubung.

    // 2. Cek Pola 4-segaris (misalnya, pola 1-4-1)
    
    // Kelompokkan berdasarkan baris dan kolom
    const facesByRow = {};
    const facesByCol = {};
    for (const {r, c} of faceCoords) {
        facesByRow[r] = (facesByRow[r] || []);
        facesByRow[r].push(c);
        facesByCol[c] = (facesByCol[c] || []);
        facesByCol[c].push(r);
    }
    
    let maxRowLen = 0;
    for (const r in facesByRow) {
        // Cek apakah ada 4 ubin atau lebih di satu baris
        if (facesByRow[r].length >= 4) {
            maxRowLen = Math.max(maxRowLen, facesByRow[r].length);
            // Cek urutan: harus berurutan, misal [1, 2, 3, 4] bukan [1, 3, 5, 7]
            const sortedCols = facesByRow[r].sort((a, b) => a - b);
            let consecutiveCount = 0;
            for (let i = 0; i < sortedCols.length - 1; i++) {
                if (sortedCols[i + 1] === sortedCols[i] + 1) {
                    consecutiveCount++;
                } else {
                    consecutiveCount = 0; // Reset jika ada gap
                }
                if (consecutiveCount >= 3) { // Berarti ada 4 ubin berurutan
                    // Cek: Apakah ubin di luar baris 4-segaris ini tidak melanggar?
                    // Jaring-jaring kubus 4-segaris (1-4-1, 2-4, 4-2) selalu valid jika terhubung.
                    // Karena kita sudah membatasi ke 5x5 dan sudah cek tumpang tindih 5+, cek 4-segaris cukup untuk validasi pola.
                    return true; 
                }
            }
        }
    ...
    let maxColLen = 0;
    for (const c in facesByCol) {
        // Cek apakah ada 4 ubin atau lebih di satu kolom
        if (facesByCol[c].length >= 4) {
             maxColLen = Math.max(maxColLen, facesByCol[c].length);
             const sortedRows = facesByCol[c].sort((a, b) => a - b);
             let consecutiveCount = 0;
    ...
    // 3. Cek Pola 3-3
    // Ini adalah heuristik yang lebih sulit, tapi kita bisa cek keberadaan 2 baris atau 2 kolom dengan 3 ubin.
    
    const rowsWithThree = Object.keys(facesByRow).filter(r => facesByRow[r].length === 3);
    const colsWithThree = Object.keys(facesByCol).filter(c => facesByCol[c].length === 3);

    // Pola 3-3 (misalnya, di 2 baris terpisah, atau 2 kolom terpisah)
    if (rowsWithThree.length === 2 && facesByRow[rowsWithThree[0]].length === 3 && facesByRow[rowsWithThree[1]].length === 3) {
        return true; // Pola 3-3 (sebaris)
    }

    if (colsWithThree.length === 2 && facesByCol[colsWithThree[0]].length === 3 && facesByCol[colsWithThree[1]].length === 3) {
        return true; // Pola 3-3 (sekolom)
    }
    
    // Pola 2-3-1 atau 3-2-1
    // Pola-pola lain yang valid akan terdeteksi di sini dengan asumsi 6 muka persegi terhubung.
    // Misalnya 1-3-1-1 (juga valid jika disusun di papan 5x5)
    
    // Cek Pola: Ubin yang tersisa setelah 4-berurutan atau 3-3 (terlalu kompleks untuk grid sederhana)
    // Untuk grid 5x5 dan 6 ubin, jika terhubung dan tidak tumpang tindih (cek 5+ sudah ada),
    // mayoritas pola 6-muka persegi yang valid pasti mengandung 4-segaris, 3-3, atau 3-2-1, yang sulit dibedakan.
    // Daripada memasukkan semua 11 pola, kita akan memvalidasi yang paling jelas (4-segaris & 3-3)
    // dan menerima pola lain karena kompleksitas cek lipatan.
    
    return maxRowLen === 3 || maxColLen === 3; // Jika tidak ada 4-segaris, coba cek 3-segaris (misal 3-3)

    // Catatan: Heuristik ini tetap tidak mencakup semua 11 pola dengan sempurna, 
    // tetapi ini adalah perbaikan dari hanya menghitung jumlah muka.
    // Jaring-jaring kubus yang memiliki lebih dari 4 ubin di satu baris/kolom lurus
    // akan terdeteksi oleh cek tumpang tindih di bawah.
}


// Fungsi utama untuk memeriksa Jaring-Jaring (Umpan Balik dengan Pop-up)
function checkNetValidity() {
    const { connected, faces, faceCoords } = isConnected(netBoardState);
    const counts = {
    ...
    // Cek Tumpang Tindih (Heuristik sederhana: lebih dari 4 ubin lurus)
    // Jaring-jaring kubus/balok/prisma/limas tidak boleh memiliki 5 ubin atau lebih yang segaris.
    let isOverlapping = false;
    // Cek Baris
    for (let i = 0; i < BOARD_SIZE_GAME; i++) {
        const rowFaces = netBoardState[i].filter(s => s !== null);
        if (rowFaces.length >= 5) {
            // Cek apakah 5 atau lebih ini berurutan
            const occupiedCols = [];
            for(let j=0; j<BOARD_SIZE_GAME; j++) {
                if (netBoardState[i][j] !== null) occupiedCols.push(j);
            }
            if (occupiedCols.length >= 5) { // Hanya perlu cek jumlah total di baris 5x5
                isOverlapping = true; 
                break;
            }
        }
    ...
    if (counts.Total === 6 && counts.Persegi === 6 && counts.PersegiPanjang === 0 && counts.Segitiga === 0) {
         if (isValidCubeNet(faceCoords)) {
            shapeName = "KUBUS";
            title = `🎉 Bangun Ruang Terbentuk: ${shapeName}!`;
            body = `6 Muka Persegi ini adalah salah satu jaring-jaring **${shapeName}** yang valid. Kerja bagus!`;
            isSuccess = true;
         } else {
            title = "⚠️ Jaring-Jaring Kubus Invalid!";
            body = "Anda telah menyusun 6 muka Persegi yang terhubung, namun pola ini tidak dapat dilipat menjadi **Kubus** yang utuh.";
         }
    } 
    // Balok (6 muka, minimal ada Persegi Panjang atau Persegi dengan ukuran berbeda)
    else if (counts.Total === 6 && counts.Segitiga === 0 && counts.PersegiPanjang >= 1) {
        // Cek Balok tidak bisa seakurat Kubus tanpa tahu dimensi sebenarnya,
        // jadi kita terima semua pola 6-muka yang tidak melanggar cek tumpang tindih
        // dan setidaknya memiliki 1 Persegi Panjang.
        shapeName = "BALOK";
        title = `✨ Bangun Ruang Terbentuk: ${shapeName}!`;
        body = `6 Muka (Persegi dan/atau Persegi Panjang) ini adalah salah satu jaring-jaring **${shapeName}** yang valid.`;
        isSuccess = true;
    } 
    // Prisma Segitiga (5 muka: 2 Segitiga, 3 Persegi/Persegi Panjang)
        ...
        shapeName = "PRISMA SEGITIGA";
        title = `💡 Bangun Ruang Terbentuk: ${shapeName}!`;
        body = `2 Alas Segitiga dan 3 Sisi Tegak (Persegi/Persegi Panjang) ini adalah jaring-jaring **${shapeName}**.`;
        isSuccess = true;
    } 
    // Limas Segiempat (5 muka: 4 Segitiga, 1 Alas)
    else if (counts.Total === 5 && counts.Segitiga === 4 && (counts.Persegi + counts.PersegiPanjang) === 1) {
        shapeName = "LIMAS SEGIEMPAT";
        title = `🧐 Bangun Ruang Terbentuk: ${shapeName}!`;
        body = `4 Sisi Tegak Segitiga dan 1 Alas (Persegi/Persegi Panjang) ini adalah jaring-jaring **${shapeName}**.`;
        isSuccess = true;
    }
    ...
    if (!isSuccess) {
        ...
        if (counts.Total === 6 && counts.Persegi === 6) {
             // Biarkan pesan kegagalan Kubus di atas muncul
        } else {
...
const quizData = [
    {
        question: "Santi ingin membuat kotak kado berbentuk kubus untuk ulang tahun temannya. Jika panjang rusuk kotak kado tersebut adalah 8 cm, berapa luas minimal kertas kado yang dibutuhkan Santi untuk membungkus seluruh permukaan kotak tersebut?",
        answers: ["A. 256 cm²", "B. 384 cm²", "C. 512 cm²", "D. 512 cm²"],
        correct: "B. 384 cm²"
    },
    {
        question: "Pak Budi membuat kolam ikan  berbentuk balok di halaman rumahnya. Kolam tersebut memiliki panjang 2OO cm, lebar 50 cm dan tinggi 80 cm. Berapa volume air (dalam cm³) maksimal yang dapat ditampung kolam tersebut jika diisi penuh?",
        answers: ["A. 80.000 cm³", "B. 8000.000 cm³", "C. 20.000 cm³", "D. 800.000 cm³"],
        correct: "D. 800.000 cm³"
    },
    {
        question: "Regu pramuka membuat tenda  yang berbentuk prisma segitiga. Alas tenda berbentuk segitiga sama kaki dengan alas 2 meter dan tinggi segitiga 1,5 meter. Jika panjang tenda (tinggi prisma) adalah 3 meter, berapa volume udara di dalam tenda tersebut?",
        answers: ["A. 4,5 m³", "B. 3,0 m³", "C. 6,0 m³", "D. 1,5 m³"],
        correct: "A. 4,5 m³"
    },
    {
        question: "Sebuah kontraktor ditugaskan membuat monumen peringatan  berbentuk limas tegak dengan alas persegi. Kontraktor berencana melapisi seluruh bidang miring (sisi tegak) monumen dengan marmer. Alas monumen memiliki panjang sisi 12 meter. Jika tinggi vertikal monumen (jarak dari puncak ke titik pusat alas) adalah 8 meter. Berapakah volume total beton yang dibutuhkan untuk membuat monumen tersebut dan Berapakah luas total marmer (dalam m²) yang dibutuhkan? Pilihan (pasangan: Volume beton ; Luas marmer):",
        answers: ["A. 288 m³ ; 192 m²", "B. 256 m³ ; 240 m²", "C. 576 m³ ; 288 m²", "D. 384 m³ ; 240 m²"],
        correct: "D. 384 m³ ; 240 m²"
    }
];
...
const timerDisplay = document.getElementById('timer'); // Elemen Timer Baru

let currentQuestionIndex = 0;
let score = 0;
let timerInterval; // Variabel untuk menyimpan interval timer

// Batasan Waktu dalam DETIK (Diperbarui untuk setiap soal)
const quizTimeLimits = [300, 300, 300, 420];
...
function updateTimerDisplay(seconds) {
    ...
    timerDisplay.textContent = `Sisa Waktu: ${minutes}:${remainderSeconds.toString().padStart(2, '0')}`;
}

function startTimer(duration) {
    if (timerInterval) clearInterval(timerInterval); // Hentikan timer sebelumnya
    ...
    timerInterval = setInterval(() => {
        ...
        if (timeRemaining <= 0) {
            ...
            // Menonaktifkan semua tombol jawaban dan menandai sebagai salah
            ...
            quizResult.textContent = "Waktu untuk soal ini habis. Skor tidak bertambah.";
...
function startQuiz() {
    ...
    if (timerInterval) clearInterval(timerInterval); // Bersihkan timer pada awal kuis
...
function showQuestion(q) {
    ...
    btnNext.classList.add("hidden");
    quizResult.textContent = ""; // Bersihkan hasil kuis dari soal sebelumnya

    // MULAI TIMER untuk pertanyaan saat ini
    const timeLimit = quizTimeLimits[currentQuestionIndex] || 150; 
...
function selectAnswer(selectedButton, correctAnswer) {
    // HENTIKAN TIMER segera setelah pengguna memilih jawaban
    if (timerInterval) clearInterval(timerInterval); 
    ...
    quizResult.textContent = ""; // Bersihkan hasil sebelumnya saat jawaban dipilih
...
function showResult() {
    // HENTIKAN TIMER saat kuis selesai
    if (timerInterval) clearInterval(timerInterval);
    timerDisplay.textContent = 'Kuis Selesai!';
